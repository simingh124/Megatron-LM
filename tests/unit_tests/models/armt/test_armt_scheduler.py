...
@pytest.mark.parametrize(
    ("no_loss_from_first_chunk", "expected_lm_loss", "expected_full_seq_loss"),
    [
        (False, torch.tensor([12.0, 8.0]), torch.tensor([12.0, 8.0])),
        (True, torch.tensor([8.0, 4.0]), torch.tensor([12.0, 8.0])),
    ],
)
def test_full_seq_loss_metric_tracks_unmasked_sequence_loss(
    no_loss_from_first_chunk, expected_lm_loss, expected_full_seq_loss
):
    ...
    def forward_step_func(data_it, model):
        ...
        def _loss_func(_output_tensor):
            num_tokens = batch["loss_mask"].float().sum().to(torch.int)
            chunk_scale = torch.tensor(float(forward_calls["n"]))
            loss_sum = num_tokens.float() * chunk_scale
    ...
    with _patched_schedule(raw_batch, config):
        ...
        try:
            training_global_vars._GLOBAL_ARGS = SimpleNamespace(
                recurrent_chunk_size=chunk_size,
                no_loss_from_first_chunk=no_loss_from_first_chunk,
            )
            losses = recurrent_forward_backward_no_pipelining(
                forward_step_func=forward_step_func,
                data_iterator=iter([raw_batch]),
    ...
    assert forward_calls["n"] == seq_length // chunk_size
    assert torch.allclose(losses[0]["lm loss"], expected_lm_loss)
    assert torch.allclose(losses[0]["train/avg_loss"], expected_full_seq_loss)
...
@pytest.mark.parametrize("no_loss_from_first_chunk", [False, True])
def test_scheduler_publishes_chunk_and_armt_monitoring_metrics(no_loss_from_first_chunk):
