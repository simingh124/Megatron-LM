...
- `hadamard` lives in sibling worktree `/mnt/step3-abla/siming/code_repo/Megatron-LM-hadamard` and currently starts from `armt` commit `35fd7ae00`. If it needs to be recreated from a newer `armt`, remove the sibling worktree first and then create the branch/worktree explicitly from the refreshed `armt` HEAD.
...
- Training-side ARMT metrics that are only available after backward must call `accumulate_armt_tensorboard_metrics()` after the recurrent scheduler publishes forward primitives. Calling `publish_armt_tensorboard_metrics()` from training.py would clear the scheduler's read/token/write metrics for that sampled step.
...
## HuggingFace Dependency Bounds
- Keep the `mlm` extra below Transformers 5.x. This repo's locked environment uses `transformers==4.57.5` with `huggingface-hub==0.36.0`; allowing Transformers 5.x can break early tokenizer imports before ARMT training starts.
- The local `${ROOT}/tokenizers/qwen3_tokenizer` directory contains only `tokenizer.json` plus `tokenizer_config.json`, not the slow Qwen2 `vocab.json`. Qwen3 launchers that use `--tokenizer-type HuggingFaceTokenizer` with this directory must include `--tokenizer-hf-use-fast`; otherwise Transformers instantiates the slow Qwen2 tokenizer and fails with `vocab_file=None`.
