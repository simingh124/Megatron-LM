...
class CrossAttentionSlotMemory(nn.Module):
    ...
    def _accumulate_monitoring_vector(self, name: str, value: torch.Tensor):
        ...
        if current is None:
            ...
            if current.shape != value.shape:
                raise ValueError(
                    "CrossAttentionSlotMemory monitoring expects stable vector shapes for "
    ...
    def update_mem(
        ...
        query = self._apply_norm(
            self._to_heads(self.W_write_q(prepared_mem_tokens)), self.write_q_norm
        )
        key = self._apply_norm(self._to_heads(self.W_write_k(slot_state)), self.write_k_norm)
        value = self._to_heads(self.W_write_v(prepared_mem_tokens))
