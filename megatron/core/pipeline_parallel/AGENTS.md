# Recurrent TBPTT Schedule Module (`megatron/core/pipeline_parallel`)
...
## Key Data Flow
1. `recurrent_forward_backward_no_pipelining(...)`（作为 `forward_backward_func` 运行）：
...
## Dev Notes
- 入口约束：PP 必须为 1（`examples/armt/armt_args.py` / `examples/recurrent/recurrent_args.py` 校验；TBPTT 仅实现 no-pipeline 路径）。
- batch 约束：
