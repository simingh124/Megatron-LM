...
# Qwen3-0.6B ARMT training with seq-level MLP-Mixer (single Linear) inserted
# between _resolve_write_source and recurrent_memory_layer.update_mem.
#
# Reference launcher:
# - playground/rmt/qwen3_0p6b_armt_gdn_0324_fs_wo_tbptt_nmem64_w_norm_post_mlp_fix_seq2048.sh
#
# Difference from the reference:
# - Enable the seq mixer via --armt-seq-mixer-type mlp1.
# - Use --armt-seq-mixer-init identity so step 0 is a strict no-op (W = I) and
#   the initial loss matches the reference launcher exactly.
# - SEQ_LENGTH (2048) is a multiple of ARMT_CHUNK_SIZE (512), so every chunk
#   is exactly 512 tokens long and the Linear(S, S) weight shape is well-defined.
#
...
# Seq mixer (single Linear S->S along the chunk axis).
ARMT_SEQ_MIXER_TYPE=${ARMT_SEQ_MIXER_TYPE:-mlp1}
ARMT_SEQ_MIXER_INIT=${ARMT_SEQ_MIXER_INIT:-identity}
...
echo "ARMT_SEQ_MIXER_TYPE=${ARMT_SEQ_MIXER_TYPE} ARMT_SEQ_MIXER_INIT=${ARMT_SEQ_MIXER_INIT}"
