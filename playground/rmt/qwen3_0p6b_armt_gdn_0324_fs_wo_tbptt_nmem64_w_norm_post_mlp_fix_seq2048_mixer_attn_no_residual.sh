...
# Qwen3-0.6B ARMT training with seq-level bidirectional self-attention mixer
# (without residual) inserted between _resolve_write_source and
# recurrent_memory_layer.update_mem.
#
# Reference launcher:
# - playground/rmt/qwen3_0p6b_armt_gdn_0324_fs_wo_tbptt_nmem64_w_norm_post_mlp_fix_seq2048.sh
#
# Difference from the reference:
# - Enable the seq mixer via --armt-seq-mixer-type attn with 4 heads, pre-norm
#   and *without* residual connection.
# - Use --armt-seq-mixer-init identity. Without residual the identity start
#   is only approximate: W_o ~ 0 yields a near-zero mixer output at step 0;
#   gradients still flow and the mixer learns from step 1.
# - The attn mixer ignores chunk size at construction time (operates on
#   runtime-known S), so any seq_length / chunk_size combination is fine.
#
# Distributed settings are configurable via env vars:
#   GPUS_PER_NODE, NUM_NODES, NODE_RANK, MASTER_ADDR, MASTER_PORT
#
# Exit interval:
#   Set EXIT_INTERVAL=10 to stop after 10 iters (used by smoke validation).
#
# Optional:
...
#   ENABLE_TEST_TRAIN_RUN=1 EXIT_INTERVAL=10 LOG_INTERVAL=1 bash <this_script>
...
# Seq mixer (bidirectional self-attention, no residual, with pre-norm).
ARMT_SEQ_MIXER_TYPE=${ARMT_SEQ_MIXER_TYPE:-attn}
ARMT_SEQ_MIXER_INIT=${ARMT_SEQ_MIXER_INIT:-megatron}
ARMT_SEQ_MIXER_ATTN_NUM_HEADS=${ARMT_SEQ_MIXER_ATTN_NUM_HEADS:-16}
ARMT_SEQ_MIXER_ATTN_BACKEND=${ARMT_SEQ_MIXER_ATTN_BACKEND:-flash}
ARMT_SEQ_MIXER_ATTN_RESIDUAL=${ARMT_SEQ_MIXER_ATTN_RESIDUAL:-0}
ARMT_SEQ_MIXER_ATTN_PRENORM=${ARMT_SEQ_MIXER_ATTN_PRENORM:-1}
...
ARMT_ARGS=(
  ...
  --armt-seq-mixer-type ${ARMT_SEQ_MIXER_TYPE}
  --armt-seq-mixer-init ${ARMT_SEQ_MIXER_INIT}
  --armt-seq-mixer-attn-num-heads ${ARMT_SEQ_MIXER_ATTN_NUM_HEADS}
  --armt-seq-mixer-attn-backend ${ARMT_SEQ_MIXER_ATTN_BACKEND}
)
if [[ "${ARMT_SEQ_MIXER_ATTN_RESIDUAL}" == "1" ]]; then
  ARMT_ARGS+=(--armt-seq-mixer-attn-residual)
else
  ARMT_ARGS+=(--no-armt-seq-mixer-attn-residual)
fi
if [[ "${ARMT_SEQ_MIXER_ATTN_PRENORM}" == "1" ]]; then
  ARMT_ARGS+=(--armt-seq-mixer-attn-prenorm)
else
  ARMT_ARGS+=(--no-armt-seq-mixer-attn-prenorm)
fi
...
echo "ARMT_SEQ_MIXER_TYPE=${ARMT_SEQ_MIXER_TYPE} ARMT_SEQ_MIXER_INIT=${ARMT_SEQ_MIXER_INIT} ARMT_SEQ_MIXER_ATTN_NUM_HEADS=${ARMT_SEQ_MIXER_ATTN_NUM_HEADS} ARMT_SEQ_MIXER_ATTN_BACKEND=${ARMT_SEQ_MIXER_ATTN_BACKEND} ARMT_SEQ_MIXER_ATTN_RESIDUAL=${ARMT_SEQ_MIXER_ATTN_RESIDUAL} ARMT_SEQ_MIXER_ATTN_PRENORM=${ARMT_SEQ_MIXER_ATTN_PRENORM}"
