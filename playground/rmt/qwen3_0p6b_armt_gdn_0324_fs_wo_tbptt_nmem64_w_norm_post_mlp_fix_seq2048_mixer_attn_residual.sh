...
# (with residual) inserted between _resolve_write_source and
...
# - Use --armt-seq-mixer-init identity. Residual + W_o = 0 gives a strict
#   no-op at step 0, so the initial loss matches the reference launcher.
...
# Seq mixer (bidirectional self-attention with residual + pre-norm).
...
ARMT_SEQ_MIXER_ATTN_RESIDUAL=${ARMT_SEQ_MIXER_ATTN_RESIDUAL:-1}
