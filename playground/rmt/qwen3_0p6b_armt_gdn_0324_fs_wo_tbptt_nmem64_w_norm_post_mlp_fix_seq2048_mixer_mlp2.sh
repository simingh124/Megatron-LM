...
# Qwen3-0.6B ARMT training with seq-level MLP-Mixer (two-layer Linear + SiLU)
# inserted between _resolve_write_source and recurrent_memory_layer.update_mem.
#
# Reference launcher:
# - playground/rmt/qwen3_0p6b_armt_gdn_0324_fs_wo_tbptt_nmem64_w_norm_post_mlp_fix_seq2048.sh
#
# Difference from the reference:
# - Enable the seq mixer via --armt-seq-mixer-type mlp2 with expansion = 2.
# - Use --armt-seq-mixer-init identity. Note: mlp2 has no residual, so the
#   identity start is approximate (fc2.weight ~ 0 yields a near-zero mixer
#   output at step 0); gradients still flow and the mixer learns from step 1.
# - SEQ_LENGTH (2048) is a multiple of ARMT_CHUNK_SIZE (512), so every chunk
#   is exactly 512 tokens long and the Linear weight shapes are well-defined.
#
...
# Seq mixer (two Linear layers S->S*e->S with SiLU between, no residual).
ARMT_SEQ_MIXER_TYPE=${ARMT_SEQ_MIXER_TYPE:-mlp2}
ARMT_SEQ_MIXER_INIT=${ARMT_SEQ_MIXER_INIT:-megatron}
ARMT_SEQ_MIXER_MLP_EXPANSION=${ARMT_SEQ_MIXER_MLP_EXPANSION:-2}
...
ARMT_ARGS=(
  ...
  --armt-seq-mixer-mlp-expansion ${ARMT_SEQ_MIXER_MLP_EXPANSION}
)
...
echo "ARMT_SEQ_MIXER_TYPE=${ARMT_SEQ_MIXER_TYPE} ARMT_SEQ_MIXER_INIT=${ARMT_SEQ_MIXER_INIT} ARMT_SEQ_MIXER_MLP_EXPANSION=${ARMT_SEQ_MIXER_MLP_EXPANSION}"
