...
# Qwen3-0.6B ARMT GDN smoke launcher for initial read-prefix concat mode with residual.
#
# Reference launcher:
# - playground/rmt/qwen3_0p6b_armt_gdn_0324_fs_wo_tbptt_nmem64_w_norm_post_mlp_fix.sh
#
# Difference from the reference:
# - Add learnable read-memory prefix tokens managed at the top-level ARMT model.
# - Use the evolving initial-layer read prefix hidden states as later-layer associate() queries.
# - Enable read-prefix residual for the selected read-memory mode: query + associate(query).
# - Keep post-MLP context writes and disable write-memory token concatenation.
# - Default to a 10-step --test-train-run smoke configuration.
# - Match the post_mlp_fix launcher sequence/batch defaults for fair throughput comparison.
#
...
#   ARMT_READ_MEMORY_RESIDUAL=0     disable query + associate(query) read-prefix residual
...
ARMT_READ_MEMORY_MODE=${ARMT_READ_MEMORY_MODE:-initial}
