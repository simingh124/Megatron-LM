...
#   PYTORCH_PROFILER_WITH_STACK=0     keep Python stack metadata out of traces
#   PYTORCH_PROFILER_GZIP_TRACES=1    gzip-compress trace JSON to reduce output size
#
# Default verification config:
#   ENABLE_TEST_TRAIN_RUN=1 with EXIT_INTERVAL=6
#   PROFILE_STEP_START=2 and PROFILE_STEP_END=5 capture training steps 3-5
...
PYTORCH_PROFILER_WITH_STACK=${PYTORCH_PROFILER_WITH_STACK:-0}
PYTORCH_PROFILER_GZIP_TRACES=${PYTORCH_PROFILER_GZIP_TRACES:-1}
...
ARMT_CHUNK_SIZE=${ARMT_CHUNK_SIZE:-256}
...
TOKEN_DERIVED_WARMUP_TOKENS=$(( 1000 * ${GLOBAL_BATCH_SIZE} * ${SEQ_LENGTH} ))
TOKEN_DERIVED_TRAIN_ITERS=$(( ${TRAIN_TOKENS} / ${GLOBAL_BATCH_SIZE} / ${SEQ_LENGTH} ))
TOKEN_DERIVED_LR_WARMUP_ITERS=$(( ${TOKEN_DERIVED_WARMUP_TOKENS} / ${GLOBAL_BATCH_SIZE} / ${SEQ_LENGTH} ))
TOKEN_DERIVED_LR_DECAY_ITERS=$(( ${LR_DECAY_TOKENS} / ${GLOBAL_BATCH_SIZE} / ${SEQ_LENGTH} ))

WARMUP_TOKENS=${WARMUP_TOKENS:-${TOKEN_DERIVED_WARMUP_TOKENS}}
TRAIN_ITERS=${TRAIN_ITERS:-${TOKEN_DERIVED_TRAIN_ITERS}}
LR_WARMUP_ITERS=${LR_WARMUP_ITERS:-${TOKEN_DERIVED_LR_WARMUP_ITERS}}
LR_DECAY_ITERS=${LR_DECAY_ITERS:-${TOKEN_DERIVED_LR_DECAY_ITERS}}
...
if [[ "${ENABLE_PYTORCH_PROFILER}" == "1" ]] && (( PROFILE_STEP_END <= PROFILE_STEP_START )); then
  echo "ERROR: PROFILE_STEP_END (${PROFILE_STEP_END}) must be greater than PROFILE_STEP_START (${PROFILE_STEP_START})" >&2
  exit 1
fi

PROFILE_RUN_STEPS=${EXIT_INTERVAL:-${TRAIN_ITERS}}
if [[ "${ENABLE_PYTORCH_PROFILER}" == "1" ]] && (( PROFILE_RUN_STEPS < PROFILE_STEP_END )); then
  echo "ERROR: profiling run length (${PROFILE_RUN_STEPS}) must be at least PROFILE_STEP_END (${PROFILE_STEP_END})" >&2
  exit 1
fi
...
if [[ "${ENABLE_PYTORCH_PROFILER}" == "1" ]]; then
  read -r -a PROFILE_RANKS_ARGS <<< "${PROFILE_RANKS}"
fi
...
if [[ "${ENABLE_PYTORCH_PROFILER}" == "1" ]]; then
  EXTRA_ARGS+=(
    ...
    --pytorch-profiler-trace-format "${PYTORCH_PROFILER_TRACE_FORMAT}"
    --profile-step-start "${PROFILE_STEP_START}"
    --profile-step-end "${PROFILE_STEP_END}"
    --profile-ranks "${PROFILE_RANKS_ARGS[@]}"
  )
  if [[ "${PYTORCH_PROFILER_RECORD_SHAPES}" == "1" ]]; then
    EXTRA_ARGS+=(--pytorch-profiler-record-shapes)
  fi
  if [[ "${PYTORCH_PROFILER_WITH_STACK}" == "1" ]]; then
    EXTRA_ARGS+=(--pytorch-profiler-with-stack)
  fi
  if [[ "${PYTORCH_PROFILER_GZIP_TRACES}" == "1" ]]; then
    EXTRA_ARGS+=(--pytorch-profiler-gzip-traces)
  fi
fi
...
echo "TRAIN_TOKENS=${TRAIN_TOKENS} TRAIN_ITERS=${TRAIN_ITERS} EXIT_INTERVAL=${EXIT_INTERVAL:-none} LR_WARMUP_ITERS=${LR_WARMUP_ITERS} LR_DECAY_ITERS=${LR_DECAY_ITERS}"
...
echo "PROFILE_STEP_START=${PROFILE_STEP_START} PROFILE_STEP_END=${PROFILE_STEP_END} PROFILE_RANKS=${PROFILE_RANKS}"
