...
- `megatron/core/pipeline_parallel/`: `armt_schedules.py`（TBPTT 调度实现；需在训练 loop 中选用）。
...
4. 由 `train.py` 进入 Megatron `pretrain()`；TBPTT 需要将 `armt_forward_backward_no_pipelining` 作为 `forward_backward_func` 运行。

## Dev Notes
- `MODEL_ARGS` 必须与 `--load` checkpoint 结构一致（层数、hidden、TP/PP 等）。
- ARMT TBPTT 依赖 `--armt-chunk-size`；`forward_step` 期望输入 batch 已在 GPU（推荐用 `armt_schedules.py` 调度）。
