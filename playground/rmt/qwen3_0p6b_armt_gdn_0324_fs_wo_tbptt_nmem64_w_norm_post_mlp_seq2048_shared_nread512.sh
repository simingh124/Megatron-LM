...
# Qwen3-0.6B ARMT GDN smoke launcher for shared read-prefix concat mode with monitor metrics.
#
# Reference launcher:
# - playground/rmt/qwen3_0p6b_armt_gdn_0324_fs_wo_tbptt_nmem64_w_norm_post_mlp_seq2048_shared.sh
#
# Difference from the reference:
# - Enable armt/read_prefix/attn_mass_from_context_mean TensorBoard monitoring by default.
#
# Distributed settings are configurable via env vars:
#   GPUS_PER_NODE, NUM_NODES, NODE_RANK, MASTER_ADDR, MASTER_PORT
#
# Optional:
#   ENABLE_RESUME=1                 load the latest checkpoint from CHECKPOINT_PATH and continue
#   NUM_READ_MEM_TOKENS=<int>       override the read-prefix token count (default: 512)
#   ARMT_LOG_LAYER_METRICS_TO_TENSORBOARD=1  emit per-layer copies of aggregated read-prefix metrics
#   ARMT_LOG_READ_POSITION_METRICS_TO_TENSORBOARD=1  emit per-position backend read metrics
#   ARMT_LOG_READ_PREFIX_ATTN_MASS_TO_TENSORBOARD=0  disable read-prefix attention mass metrics
#
# Default verification config:
#   ENABLE_TEST_TRAIN_RUN=1
#   EXIT_INTERVAL=10
#   SEQ_LENGTH=2048 MICRO_BATCH_SIZE=6 GLOBAL_BATCH_SIZE=240
...
ARMT_LOG_READ_PREFIX_ATTN_MASS_TO_TENSORBOARD=${ARMT_LOG_READ_PREFIX_ATTN_MASS_TO_TENSORBOARD:-1}
...
# EXIT_INTERVAL=${EXIT_INTERVAL:-10}
...
NUM_READ_MEM_TOKENS=${NUM_READ_MEM_TOKENS:-512}
ARMT_READ_MEMORY_MODE=${ARMT_READ_MEMORY_MODE:-shared}
ARMT_READ_MEMORY_RESIDUAL=${ARMT_READ_MEMORY_RESIDUAL:-0}
...
ARMT_ARGS=(
  ...
  --num-read-mem-tokens ${NUM_READ_MEM_TOKENS}
  --armt-read-memory-mode ${ARMT_READ_MEMORY_MODE}
...
if [[ "${ARMT_READ_MEMORY_RESIDUAL}" == "1" ]]; then
  ARMT_ARGS+=(--armt-read-memory-residual)
else
  ARMT_ARGS+=(--no-armt-read-memory-residual)
fi
...
if [[ "${ARMT_LOG_READ_PREFIX_ATTN_MASS_TO_TENSORBOARD}" == "1" ]]; then
  ARMT_ARGS+=(--armt-log-read-prefix-attn-mass-to-tensorboard)
else
  ARMT_ARGS+=(--no-armt-log-read-prefix-attn-mass-to-tensorboard)
fi
...
echo "NUM_MEM_TOKENS=${NUM_MEM_TOKENS} NUM_READ_MEM_TOKENS=${NUM_READ_MEM_TOKENS} ARMT_CHUNK_SIZE=${ARMT_CHUNK_SIZE}"
echo "ARMT_READ_MEMORY_MODE=${ARMT_READ_MEMORY_MODE}"
echo "ARMT_READ_MEMORY_RESIDUAL=${ARMT_READ_MEMORY_RESIDUAL}"
...
echo "ARMT_LOG_READ_PREFIX_ATTN_MASS_TO_TENSORBOARD=${ARMT_LOG_READ_PREFIX_ATTN_MASS_TO_TENSORBOARD}"
